...
enum {
	EVOMIN_FRAME_SOF = 0xAA, EVOMIN_FRAME_EOF = 0x55, EVOMIN_FRAME_STFBYT = 0x55
};
...
static int8_t buffer_initialize(struct evoMin_Buffer* buffer, uint32_t size, int8_t allocMem);
...
evoMin_sendFrame(struct evoMin_Interface* interface, uint8_t command, uint8_t* bytes, uint32_t bLength)
{
	...
	/* 0	command
	 1	len
	 2..n payload data */
	uint8_t* crcBuffer = calloc(crcBufSize, sizeof(uint8_t));
	if(!crcBuffer) {
		return bytesToSend;
	}
	crcBuffer[0] = command;
...
static 
int8_t buffer_initialize(struct evoMin_Buffer* buffer, uint32_t size, int8_t allocateMem) 
{
	/* Only allocate memory, if the flag is set */
  	if(allocateMem == 1)
	{
...
static 
int8_t buffer_push(struct evoMin_Buffer* buffer, uint8_t byte) 
{
...
static 
void initialize_frame(struct evoMin_Frame* frame) 
{
	frame->isValid = -1;
	...
	int8_t allocMem = (frame->isInitialized == 1) ? -1 : 1;
	int8_t initState = buffer_initialize(&frame->buffer, EVOMIN_P_BUF_SIZE+2, allocMem);
	frame->isInitialized = (initState == 1) ? 1 : -1;
